...
void MapBuilderBridge::HandleSubmapQuery(
  ...
  if (!error.empty()) {
    LOG(ERROR) << error;
    response->status.code = cartographer_slam::msg::StatusCode::NOT_FOUND;
...
cartographer_slam::msg::SubmapList MapBuilderBridge::GetSubmapList(rclcpp::Time node_time) {
  cartographer_slam::msg::SubmapList submap_list;
  submap_list.header.stamp = node_time;
  submap_list.header.frame_id = node_options_.map_frame;
  for (const auto& submap_id_pose :
       map_builder_->pose_graph()->GetAllSubmapPoses()) {
    cartographer_slam::msg::SubmapEntry submap_entry;
