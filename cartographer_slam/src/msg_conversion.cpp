...
using ::cartographer_slam::msg::LandmarkEntry;
using ::cartographer_slam::msg::LandmarkList;
...
std::tuple<::cartographer::sensor::PointCloudWithIntensities,
           ::cartographer::common::Time>
ToPointCloudWithIntensities(const livox_driver::msg::CustomMsg& msg) {
  PointCloudWithIntensities point_cloud;
  ...
  // Create a sorted index array by offset_time to ensure monotonic timestamps
  std::vector<size_t> sorted_indices(num_points);
  std::iota(sorted_indices.begin(), sorted_indices.end(), 0);
  std::sort(sorted_indices.begin(), sorted_indices.end(),
            [&msg](size_t a, size_t b) {
              return msg.points[a].offset_time < msg.points[b].offset_time;
            });

  point_cloud.points.reserve(num_points);
  point_cloud.intensities.reserve(num_points);

  // Get the maximum offset_time (from the last sorted point)
  const uint32_t max_offset = msg.points[sorted_indices.back()].offset_time;
  const double max_offset_sec = static_cast<double>(max_offset) * 1e-9;

  // Process points in sorted order
  for (const size_t idx : sorted_indices) {
    const auto& point = msg.points[idx];
    // Relative time: current point time - max point time (so last point has time 0)
    const double offset_sec = static_cast<double>(point.offset_time) * 1e-9;
    const float relative_time = static_cast<float>(offset_sec - max_offset_sec);

    point_cloud.points.push_back(
        {Eigen::Vector3f{point.x, point.y, point.z}, relative_time});
    point_cloud.intensities.push_back(static_cast<float>(point.reflectivity));
  }

  // Timestamp is the time of the last point (header.stamp + max_offset)
  ::cartographer::common::Time timestamp = FromRos(msg.header.stamp);
  timestamp += cartographer::common::FromSeconds(max_offset_sec);
