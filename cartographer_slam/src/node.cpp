...
#include "cartographer_slam/metrics/family_factory.h"
#include "cartographer_slam/msg_conversion.h"
#include "cartographer_slam/sensor_bridge.h"
#include "cartographer_slam/tf_bridge.h"
#include "cartographer_slam/time_conversion.h"
#include "cartographer_slam/msg/status_code.hpp"
#include "cartographer_slam/msg/status_response.hpp"
    ...
    : node_options_(node_options)
{
  ...
  submap_list_publisher_ =
      node_->create_publisher<::cartographer_slam::msg::SubmapList>(
          kSubmapsTopic, 10);
  trajectory_node_list_publisher_ =
      node_->create_publisher<::visualization_msgs::msg::MarkerArray>(
          kTrajectoryNodesTopic, 10);
  landmark_poses_list_publisher_ =
      node_->create_publisher<::visualization_msgs::msg::MarkerArray>(
          kLandmarkPosesTopic, 10);
  constraint_list_publisher_ =
      node_->create_publisher<::visualization_msgs::msg::MarkerArray>(
          kConstraintsTopic, 10);
  ...
  submap_query_server_ = node_->create_service<cartographer_slam::srv::SubmapQuery>(
      kSubmapQueryServiceName,
      std::bind(
          &Node::handleSubmapQuery, this, std::placeholders::_1, std::placeholders::_2));
  trajectory_query_server = node_->create_service<cartographer_slam::srv::TrajectoryQuery>(
      kTrajectoryQueryServiceName,
      std::bind(
          &Node::handleTrajectoryQuery, this, std::placeholders::_1, std::placeholders::_2));
  start_trajectory_server_ = node_->create_service<cartographer_slam::srv::StartTrajectory>(
      kStartTrajectoryServiceName,
      std::bind(
          &Node::handleStartTrajectory, this, std::placeholders::_1, std::placeholders::_2));
  finish_trajectory_server_ = node_->create_service<cartographer_slam::srv::FinishTrajectory>(
      kFinishTrajectoryServiceName,
      std::bind(
          &Node::handleFinishTrajectory, this, std::placeholders::_1, std::placeholders::_2));
  write_state_server_ = node_->create_service<cartographer_slam::srv::WriteState>(
      kWriteStateServiceName,
      std::bind(
          &Node::handleWriteState, this, std::placeholders::_1, std::placeholders::_2));
  get_trajectory_states_server_ = node_->create_service<cartographer_slam::srv::GetTrajectoryStates>(
      kGetTrajectoryStatesServiceName,
      std::bind(
          &Node::handleGetTrajectoryStates, this, std::placeholders::_1, std::placeholders::_2));
  read_metrics_server_ = node_->create_service<cartographer_slam::srv::ReadMetrics>(
...
bool Node::handleSubmapQuery(
    const cartographer_slam::srv::SubmapQuery::Request::SharedPtr request,
    cartographer_slam::srv::SubmapQuery::Response::SharedPtr response) {
  absl::MutexLock lock(&mutex_);
...
bool Node::handleTrajectoryQuery(
    const cartographer_slam::srv::TrajectoryQuery::Request::SharedPtr request,
    cartographer_slam::srv::TrajectoryQuery::Response::SharedPtr response) {
  absl::MutexLock lock(&mutex_);
  response->status = TrajectoryStateToStatus(
      request->trajectory_id,
      {TrajectoryState::ACTIVE, TrajectoryState::FINISHED,
       TrajectoryState::FROZEN} /* valid states */);
  if (response->status.code != cartographer_slam::msg::StatusCode::OK) {
...
void Node::PublishLocalTrajectoryData() {
  ...
  for (const auto& entry : map_builder_bridge_->GetLocalTrajectoryData()) {
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      ...
      // Publish FAST-LIO compatible odometry message
      if (node_options_.publish_odometry) {
        nav_msgs::msg::Odometry odometry_msg;
        odometry_msg.header.frame_id =
            trajectory_data.trajectory_options.odom_frame;  // "camera_init"
        odometry_msg.child_frame_id =
            trajectory_data.trajectory_options.tracking_frame;  // "body"
        odometry_msg.header.stamp = stamped_transform.header.stamp;
        odometry_msg.pose.pose = ToGeometryMsgPose(tracking_to_map);
        // Zero covariance (not computed by Cartographer)
        std::fill(odometry_msg.pose.covariance.begin(),
                  odometry_msg.pose.covariance.end(), 0.0);
        std::fill(odometry_msg.twist.covariance.begin(),
                  odometry_msg.twist.covariance.end(), 0.0);
...
Node::ComputeExpectedSensorIds(const TrajectoryOptions& options) const {
  ...
  for (const std::string& topic :
       ComputeRepeatedTopicNames(kLivoxPointsTopic, options.num_livox_points)) {
    expected_topics.insert(SensorId{SensorType::RANGE, topic});
  }
...
void Node::LaunchSubscribers(const TrajectoryOptions& options,
  ...
  for (const std::string& topic :
       ComputeRepeatedTopicNames(kLivoxPointsTopic, options.num_livox_points)) {
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<livox_driver::msg::CustomMsg>(
             &Node::HandleLivoxPointsMessage, trajectory_id, topic, node_, this),
         topic});
  }
  ...
  if (options.use_landmarks) {
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<cartographer_slam::msg::LandmarkList>(
...
cartographer_slam::msg::StatusResponse Node::TrajectoryStateToStatus(
  ...
  cartographer_slam::msg::StatusResponse status_response;
  ...
  if (it == trajectory_states.end()) {
    ...
    status_response.code = cartographer_slam::msg::StatusCode::NOT_FOUND;
  ...
  status_response.code =
      valid_states.count(it->second)
          ? cartographer_slam::msg::StatusCode::OK
          : cartographer_slam::msg::StatusCode::INVALID_ARGUMENT;
  return status_response;
}

cartographer_slam::msg::StatusResponse Node::FinishTrajectoryUnderLock(
    const int trajectory_id) {
  cartographer_slam::msg::StatusResponse status_response;
  if (trajectories_scheduled_for_finish_.count(trajectory_id)) {
    status_response.message =
        "Trajectory " + std::to_string(trajectory_id) + " already pending to finish.";
    status_response.code = cartographer_slam::msg::StatusCode::OK;
  ...
  if (status_response.code != cartographer_slam::msg::StatusCode::OK) {
...
bool Node::handleStartTrajectory(
    const cartographer_slam::srv::StartTrajectory::Request::SharedPtr request,
    cartographer_slam::srv::StartTrajectory::Response::SharedPtr response) {
  ...
  if (!ValidateTrajectoryOptions(trajectory_options)) {
    ...
    LOG(ERROR) << response->status.message;
    response->status.code = cartographer_slam::msg::StatusCode::INVALID_ARGUMENT;
    ...
    response->status.code = cartographer_slam::msg::StatusCode::OK;
  }
  return true;
}
...
bool Node::handleGetTrajectoryStates(
    const cartographer_slam::srv::GetTrajectoryStates::Request::SharedPtr ,
    cartographer_slam::srv::GetTrajectoryStates::Response::SharedPtr response) {
    ...
    switch (entry.second) {
      case TrajectoryState::ACTIVE:
        response->trajectory_states.trajectory_state.push_back(
            ::cartographer_slam::msg::TrajectoryStates::ACTIVE);
        break;
      case TrajectoryState::FINISHED:
        response->trajectory_states.trajectory_state.push_back(
            ::cartographer_slam::msg::TrajectoryStates::FINISHED);
        break;
      case TrajectoryState::FROZEN:
        response->trajectory_states.trajectory_state.push_back(
            ::cartographer_slam::msg::TrajectoryStates::FROZEN);
        break;
      case TrajectoryState::DELETED:
        response->trajectory_states.trajectory_state.push_back(
            ::cartographer_slam::msg::TrajectoryStates::DELETED);
        break;
    }
  }
  return true;
...
bool Node::handleFinishTrajectory(
    const cartographer_slam::srv::FinishTrajectory::Request::SharedPtr request,
    cartographer_slam::srv::FinishTrajectory::Response::SharedPtr response) {
  absl::MutexLock lock(&mutex_);
...
bool Node::handleWriteState(
    const cartographer_slam::srv::WriteState::Request::SharedPtr request,
    cartographer_slam::srv::WriteState::Response::SharedPtr response) {
  absl::MutexLock lock(&mutex_);
...
bool Node::handleReadMetrics(
    const cartographer_slam::srv::ReadMetrics::Request::SharedPtr,
    cartographer_slam::srv::ReadMetrics::Response::SharedPtr response) {

  absl::MutexLock lock(&mutex_);
  response->timestamp = node_->now();
  if (!metrics_registry_) {
    response->status.code = cartographer_slam::msg::StatusCode::UNAVAILABLE;
...
void Node::HandleLandmarkMessage(
    ...
    const cartographer_slam::msg::LandmarkList::ConstSharedPtr& msg) {
  absl::MutexLock lock(&mutex_);
...
void Node::HandleLivoxPointsMessage(
    const int trajectory_id, const std::string& sensor_id,
    const livox_driver::msg::CustomMsg::ConstSharedPtr& msg) {
  absl::MutexLock lock(&mutex_);
  if (!sensor_samplers_.at(trajectory_id).rangefinder_sampler.Pulse()) {
    return;
  }
  map_builder_bridge_->sensor_bridge(trajectory_id)
      ->HandleLivoxPointsMessage(sensor_id, msg);
}
