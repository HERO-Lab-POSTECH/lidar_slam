...
void CheckTrajectoryOptions(const TrajectoryOptions& options) {
  ...
  CHECK_GE(options.num_laser_scans + options.num_multi_echo_laser_scans +
               options.num_point_clouds + options.num_livox_points,
           1)
      << "Configuration error: 'num_laser_scans', "
         "'num_multi_echo_laser_scans', 'num_point_clouds', and "
         "'num_livox_points' are all zero, but at least one is required.";
}
...
TrajectoryOptions CreateTrajectoryOptions(
  ...
  options.num_livox_points =
      lua_parameter_dictionary->GetNonNegativeInt("num_livox_points");
